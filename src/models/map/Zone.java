...
public class Zone {
    ...
    public Player getRandomPlayerInMap() {
        // Sử dụng kiểu dữ liệu rõ ràng
        List<Player> plNotVoHinh = new ArrayList<>();

        // Duyệt qua danh sách notBosses và lọc các player hợp lệ
        for (Player pl : this.notBosses) {
            // Kiểm tra các điều kiện cho player
            if (pl != null && (pl.effectSkin == null || !pl.effectSkin.isVoHinh) && pl.maBuHold == null && !pl.isMabuHold) {
                plNotVoHinh.add(pl);
            }
        }

        // Kiểm tra nếu danh sách plNotVoHinh không rỗng và lấy ngẫu nhiên player
        if (!plNotVoHinh.isEmpty()) {
            // Kiểm tra điều kiện để đảm bảo nextInt không gây lỗi khi size() = 1
            int randomIndex = Util.nextInt(0, plNotVoHinh.size() - 1);
            return plNotVoHinh.get(randomIndex); // Lấy player ngẫu nhiên
        }

        return null; // Nếu danh sách trống, trả về null
    }
