        ...
        if (isTimeSupportTask(TimeUtil.getCurrHour()) && zoneJoin != null) {
            for (Player b : zoneJoin.getBosses()) {
                if ((b.id == BossID.SO_4 || b.id == BossID.SO_3 || b.id == BossID.SO_2 || b.id == BossID.SO_1 || b.id == BossID.TIEU_DOI_TRUONG)
                        ...
                        || (b.id == BossID.FIDE)
                        && (TaskService.gI().getIdTask(player) < ConstTask.TASK_21_1 || TaskService.gI().getIdTask(player) > ConstTask.TASK_21_3)
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {
                ...
                case 155://hành tinh ngục tù
                    if (player.nPoint.power < 60_000_000_000L || TaskService.gI().getIdTask(player) < ConstTask.TASK_26_0) {
                        return null;
                    }
                    break;
            }
