...
//            }
    ...
    public void checkDoneTaskKillBoss(Player player, Boss boss) {
        if (player != null && !player.isBoss && !player.isPet && !player.isBot) {
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_11_0:
                    npcSay(player, ConstNpc.LY_TIEU_NUONG, "Chị là nhà cái uy tín đến từ Đảo Kame\n"
    ...
    private void rewardDoneTask(Player player) {
        switch (player.playerTask.taskMain.id) {
            ...
            case 13:
                ...
                InventoryService.gI().addItemBag(player, item);
                InventoryService.gI().sendItemBags(player);
                Service.gI().sendThongBao(player, "Chúc mừng bạn vừa nhận được đuôi khỉ!");
                break;
            case 33:
            ...
            if (player.playerTask.sideTask.isDone()) {
                ...
                if (InventoryService.gI().getCountEmptyBag(player) > 1) {
                    if (cayThong != -1 && canNhanCayThong) {
                        ...
                        Item day = ItemService.gI().createNewItem((short) daykimtuyen);
                        ...
                        InventoryService.gI().addItemBag(player, day);
                        ...
                        Service.gI().sendThongBao(player, "Bạn nhận được " + day.template.name);
                    }
                    ...
                    Service.gI().sendThongBao(player, "Bạn nhận được "
                            + Util.numberToMoney(gemReward) + "ngọc xanh");
                    // Gọi addPointNongDanChamChi để tăng điểm cho người chơi
                    player.effect.addPointNongDanChamChi();  // Thêm điểm sau mỗi nhiệm vụ hoàn thành

                    player.playerTask.sideTask.reset();  // Đặt lại trạng thái nhiệm vụ
                } else {
