...
//    public long giaSo_Gold = 1_000_000; // vàng
    ...
    public void newData(Player player, int point, int VangorNgoc) { // ngọc // vang = 0; ngoc = 1;
        ...
        if (VangorNgoc == 0) {
            if (thoiVang == null) {
                Service.gI().sendThongBao(player, "Bạn có thỏi vàng lồn đâu????");
                return;
            } else if (thoiVang.quantity < this.giaSo_Gold) {
                Service.gI().sendThongBao(player, "Bạn có đủ thỏi vàng lồn đâu????");
                return;
            }
...
//            }
        ...
        if (VangorNgoc == 1) {
            if (!players.stream().anyMatch(game -> game.id == player.id && game.point == point)) {
                ...
                Service.gI().sendThongBao(player, "Bạn bị trừ " + thoiVang.quantity + " thỏi vàng vì cờ bạc");
    ...
    public String strFinish(int id) {
        ...
        for (ConSoMayManData g : players) {
            ...
            if (id == g.id && g.point == result && g.conSoMayManVang == 1) {
                ...
                finish = "Chúc mừng " + (player == null ? "NULL" : player.name) + " đã thắng " + Util.mumber(MiniGame.gI().MiniGame_S1.gold) + " thỏi vàng với con số may mắn " + result;
                // trả thưởng cho player trúng thưởng //
